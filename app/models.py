...
class ComponentFailure(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    ler_id = db.Column(db.Integer, db.ForeignKey('lers.id'))
...
class FacilityInvolved(db.Model):
    __tablename__ = 'facilitiesinvolved'
    id = db.Column(db.Integer, primary_key=True)
    ler_id = db.Column(db.Integer, db.ForeignKey('lers.id'))
    facility_id = db.Column(db.Integer, db.ForeignKey('facilities.id'))

    def __repr__(self):
        return '<FacilityInvolved(%r, %r, %r)>' % self.id, self.ler_id, self.facility_id


class CFRSelected(db.Model):
    __tablename__ = 'cfrselected'
    id = db.Column(db.Integer, primary_key=True)
    ler_id = db.Column(db.Integer, db.ForeignKey('lers.id'))
    cfr_id = db.Column(db.Integer, db.ForeignKey('cfrs.id'))

    def __repr__(self):
        return '<CFRSelected(%r, %r, %r)>' % self.id, self.ler_id, self.cfr_id
